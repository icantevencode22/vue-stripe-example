...
module.exports = function (api) {
  ...
  api.configureServer((app) => {
    ...
    app.get("/get-subscription/:subscriptionId", jsonParser, async (req, res) => {
      try {
    ...
    app.post("/update-subscription/:subscriptionId", jsonParser, async (req, res) => {
      try {
        const { subscriptionId } = req.params;
        const { subscription } = req.body;
        const resUpdate = await stripe.subscriptions.update(subscriptionId, { ...subscription });
        res.send(resUpdate);
      } catch (error) {
        res.send(false);
      }
